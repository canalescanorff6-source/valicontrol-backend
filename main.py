...
# =========================
@app.post("/produtos")
def adicionar_produto(p: Produto, email=Depends(validar_token)):
    conn = conectar()
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s)
    """, (p.codigo, p.nome, p.validade, p.quantidade, email))
...
@app.get("/produtos")
def listar_produtos(email=Depends(validar_token)):
    conn = conectar()
    cursor = conn.cursor()
...
@app.put("/produtos/{id}")
def atualizar_produto(id: int, p: Produto, email=Depends(validar_token)):
    conn = conectar()
    cursor = conn.cursor()
    ...
    """, (p.codigo, p.nome, p.validade, p.quantidade, id, email))
...
def excluir_produto(id: int, email=Depends(validar_token)):
    conn = conectar()
    cursor = conn.cursor()

    cursor.execute("DELETE FROM produtos WHERE id=%s AND user_email=%s", (id, email))
